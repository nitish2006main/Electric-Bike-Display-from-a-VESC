...
void LCD::initST7789() {
  ...
  write_cmd(ST7789_MADCTL, madctl, 1);
  
  { // Color Mode
    uint8_t color_mode[] = { COLOR_MODE_65K | COLOR_MODE_16BIT};
    write_cmd(ST7789_COLMOD, color_mode, 1);
  }
...
void LCD::setWindow(int x_start, int y_start, int x_end, int y_end) {
  ...
  y_end += LCD_OFFSET_Y;
  {
    ...
    this->write_cmd(ST7789_CASET, bufx, 4);
  }

  {
    ...
    this->write_cmd(ST7789_RASET, bufy, 4);
  }
}
...
void LCD::drawBitmap(int x_start, int y_start, int x_end, int y_end, uint16_t* color_data) {
  ...
  // this->write_cmd(ST7789_RAMWR, (uint8_t*) color_data, (x_end - x_start + 1) * (y_end - y_start + 1) * 2);
  
  digitalWrite(LCD_CS_PIN, LOW);

  // Start write command
  digitalWrite(LCD_DC_PIN, LOW);
  this->write_spi(ST7789_RAMWR);
...
void LCD::drawPixel(uint16_t x, uint16_t y, uint16_t color) {
  ...
  this->write_cmd(ST7789_RAMWR, buff, sizeof(buff));
}
...
// Display OFF
void LCD::off() {
	this->write_cmd(ST7789_DISPOFF);	//Display off
}
 
// Display ON
void LCD::on() {
	this->write_cmd(ST7789_DISPON);	//Display on
}

// Fill screen
// color:color
void LCD::fillScreen(uint16_t color) {
	this->fillRect(0, 0, LCD_WIDTH - 1, LCD_HEIGHT - 1, color);
}
